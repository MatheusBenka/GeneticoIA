...
#from tkinter import *
#import pyexcel as pods
import json

#futura implementacao de imagem
'''
class auxImage(object):
    def carregarGene(self):
        rand = rnd.randint(1, 16)
        novoGene = Image.open('tile' + rand + '.png')
        return novoGene
'''
...
class auxCriacaoGene(object):
    ...
    def calcFitnessCaminhosCima(self, individuo):#algoritmo para calcular os caminhos do labirinto
        ...
        if individuo.labirinto[0][0][3] == 1: #entrada aberta na entrada do labirinto
            ...
            return 0
        '''
        if individuo.labirinto[19][19][5]: #se apos a busca visitou a ultima peca == percorreu o percurso
            individuo.caminhoValido = True #marca como tem caminho valido
            #individuo.printar('aqui')
            print('*'*10+'puta merda',end='*'*10)
        '''
    ...
    def calcularFitnessBaixo(self,individuo):
        ...
        maiorCaminhoCima = 0
        if individuo.labirinto[19][19][1] == 1:  # saida aberta na saida do labirinto
            maiorCaminhoEsquerda = 1
            maiorCaminhoCima = 1
            ## mudar linha e coluna para nove para testes
            individuo.labirinto[19][19][5] = True  # marca como visitado
            if individuo.labirinto[19][19][3] == 1:  # se a saida da esquerda estiver aberda
                maiorCaminhoEsquerda = maiorCaminhoEsquerda + self.buscaProfundidade(individuo.labirinto, 19, 18, 1,individuo.labirinto[19][18][4])
            if individuo.labirinto[19][19][0] == 1:  # se a saida para cima estiver aberta
                maiorCaminhoCima = maiorCaminhoCima + self.buscaProfundidade(individuo.labirinto, 18, 19, 2, individuo.labirinto[18][19][4])
        else:
            return 0
        '''
        if individuo.labirinto[0][0][5]:  # se apos a busca visitou a ultima peca == percorreu o percurso
            individuo.caminhoValido = True  # marca como tem caminho valido
            print('*' * 10 + 'puta merda', end='*' * 10)
        '''
    ...
    def buscaProfundidade(self,labirinto,linha,coluna,entrada,tipo):
        ...
        if labirinto[linha][coluna][entrada] == 1: # se tiver entrada por onde a peca anterior saiu
            ...
            if labirinto[linha][coluna][5] is False: # nao foi visitado
                ...
                if labirinto[linha][coluna][0] == 1:  # se estiver aberto a face de cima da peca
                ...
                if labirinto[linha][coluna][1] == 1:  # se estiver aberto a face da direita da peca
                ...
                if labirinto[linha][coluna][2] == 1:  # se estiver aberto a face de baixo da peca
                ...
                if labirinto[linha][coluna][3] == 1:  # se estiver aberto a face da esquerda da peca
    ...
    def printar(self,nome):
        ...
        for i in range(0, self.colunas):
            for j in range(0, self.linhas):
                #print("{}".format(self.labirinto[i][j]), end='')
...
class ambiente(object):
    ...
    def cruzamento(self):
        ...
        while qtdCruzamentos > 0:
            ...
            pontoCorteLinha = rnd.randint(0,20) #seleciona ponto de corte aleatorio
            #pontoCorteColuna = rnd.randint(0,20) #seleciona ponto de corte aleatorio
            ...
            '''if self.geracao == 10:
                self.aux.telinha(filho1.labirinto,'filho1SEMMut')
                self.aux.telinha(filho2.labirinto,'filho2SEMMut')
                self.aux.telinha(self.pai1.labirinto,'pai1')
                self.aux.telinha(self.pai2.labirinto,'pai2')
            '''
            self.mutacao(filho1)
            self.mutacao(filho2)
            '''
                if self.geracao == 10:
                    self.aux.telinha(filho1.labirinto, 'filho1comMut')
                    self.aux.telinha(filho2.labirinto, 'filho2comMut')
                    input('pausa')
            '''
            '''
            # copia genes dos pais para os filhos
            for i in range(0, pontoCorteLinha):
                for j in range(pontoCorteColuna,len(filho1)):
                    filho1.labirinto[i][j] = self.pai1.labirinto[i][j]
                    filho2.labirinto[i][j] = self.pai2.labirinto[i][j]

            for i in range(0, pontoCorteLinha):
                for j in range(0,pontoCorteColuna):
                    filho1.labirinto[i][j] = self.pai2.labirinto[i][j]
                    filho2.labirinto[i][j] = self.pai1.labirinto[i][j]
    ...
    def mediaPop(self):
        ...
        print(media/len(self.populacao))
    ...
    def mutacao(self,novo):
        ...
        # gera mutacoes
        while mutacoes > 0:
            linha = rnd.randint(0, 20)
            coluna = rnd.randint(0, 20)
            novoGene = self.aux.gerar()

            #fechar lados para mutacao
            if linha == 0 and coluna != 0:
                novoGene[0] = 0
            elif linha == 19 and coluna != 19:
                novoGene[2] = 0
            elif coluna == 0 and linha != 0:
                novoGene[3] = 0
            elif coluna == 19 and linha != 19:
                novoGene[1] = 0
    ...
    def mostrarResultados(self):
       ...
       # print('fitness da populacao = '+str(self.fitness))
        print('num geracao = '+str(self.geracao))
        #print('sem melhora = '+ str(self.iteracaoSemMelhora))
...
novoAmbiente.rodar(1000)
'''
for i in range(0,len(populacao)):
    print('individuo {}'.format(i))
    populacao[i].printar(i)
    print('antes {}'.format(populacao[i].fitness))
    aux.calcFitnessCaminhos(populacao[i])
    print('depois {}'.format(populacao[i].fitness))
'''
'''''


# gestao da imagem
#img = Image.open('tiles.png','r')
#img.rotate(270).show()
#img.close()
'''
'''
# funcao para gerar valoracao de caminhos de relacao entre pecas mais aptas
    def pegarCusto(self,qual,cima,direita,baixo,esquerda):
        if qual < 6:
            custos = pods.get_data('custos'+str(qual)+'.ods')
            #print(json.dumps(custos)) printa formatado
            fitnessPeca = 0
            if cima != 0:
                fitnessPeca = fitnessPeca + custos['Planilha1'][0][cima]
            if direita != 0:
                fitnessPeca = fitnessPeca + custos['Planilha1'][1][direita]
            if baixo != 0:
                fitnessPeca = fitnessPeca + custos['Planilha1'][2][baixo]
            if esquerda != 0:
                fitnessPeca = fitnessPeca + custos['Planilha1'][3][esquerda]
            return fitnessPeca
        return 0
'''